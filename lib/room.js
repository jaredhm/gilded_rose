...
Room.prototype.canAccomodate = function(guests) {
  let openLuggageSlots = this.luggageLimit - this.numLuggage;
  let openBeds = this.beds.filter(function(bed) {
    return bed.empty();
  });
  let viableBeds = [];
  // Long story short: sort the guests by number of pieces of luggage,
  // with the more loaded-up guests being at the lower indeces
  const sortedGuests = guests.sort(function(guestA, guestB) {
    // if B > A, return a positive number, B will be stuck at lower index
    // if B = A, return 0, relative index doesn't matter
    // if B < A, return a negative number, A will be at a lower index
    return guestB.luggage.length - guestA.luggage.length;
  });
  // Run through the guests in sorted order. If we can, with the number
  // of open luggage slots we have, fill a bed, do it.
