...
function getAccomodations(numGuests, numLuggage, inn, bookIt = false) {
  ...
  if(filledBeds.length == numGuests) {
    ...
    filledBeds.forEach(function(bed) {
      ...
      if(room in accomodations.rooms) { 
        accomodations.rooms[room].beds.push(bed.toMap())
      } else {
        // Otherwise, create value at that key
        accomodations.rooms[room] = { beds: [ bed.toMap() ] };
      }
      // No do pricing if we're booking the room
      if(bookIt) {
        //accomodations
      }
    });
