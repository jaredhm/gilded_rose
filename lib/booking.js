...
/*
 * Entry point into the booking libraries.
 * Given a number of guests, the storage constraints, the inn object, and
 * and optional bookIt flag does:
 *  - computes the most efficient set of beds that can satisfy the request
 *  - computes the cost of the booking
 *  - update the schedule
 *
 * returns:
 *  - if a suitable assignment of beds exists, the beds as a JSON-ifiable object
 *  - else, undef
 */
function getAccomodations(numGuests, numLuggage, inn, bookIt = false) {
  ...
  if(! bookIt) {
    ...
    // TODO: This should really be done when we're creating the accomodations
    // object so that we can return the checkout time as part of the response body
